...
public class FacturacionService {
	...
	public FacturacionService(FamiliaDAO familiaDAO, ConceptoDAO conceptoDAO, AlumnoRepository alumnoRepository, TallerDAO tallerDAO,
	...
	public void facturar(FacturacionRequestDTO request) {
		...
		for (Familia familia : familias) {
			...
			List<Optional<Alumno>> alumnos = familia.getMiembros().stream()
					...
					.map(miembro -> alumnoRepository.findById(miembro.getId()))
					.toList();
			// Factura
	...
	public List<DeudaDTO> obtenerDeudas(String dni) throws ServiceException
	{
		Optional<Padre> padre = padreDAO.findPadreByPersona_Dni(dni);
		if (padre.isEmpty()) throw new ServiceException("Padre no encontrado", 404);
		return obtenerDeudas().stream().filter(deuda -> deuda.getIdFamilia().equals(padre.get().getPersona().getFamilia().getId())).toList();
	}
}
