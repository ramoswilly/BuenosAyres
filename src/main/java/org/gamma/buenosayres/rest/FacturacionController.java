...
import org.gamma.buenosayres.service.FacturacionService;
...
public class FacturacionController {
	...
	@GetMapping("/deudas")
	public ResponseEntity<?> obtenerDeudas(Authentication authentication)
	{
		if (authentication.getAuthorities().stream().anyMatch(auth -> auth.getAuthority().equals("ROLE_PADRE"))) {
			try {
				return ResponseEntity.ok(facturacionService.obtenerDeudas(authentication.getName()));
			} catch (ServiceException e) {
				return ResponseEntity.status(e.getCode()).body(e.getMessage());
			}
		}
